...
/**
 * The listener interface for receiving playerPacket events.
 * The class that is interested in processing a playerPacket
 * event implements this interface, and the object created
 * with that class is registered with a component using the
 * component's <code>addPlayerPacketListener<code> method. When
 * the playerPacket event occurs, that object's appropriate
 * method is invoked.
 *
...
//			System.out.println("PPL: PlayerPacketListener Art: " + b.getNamespace() + " Type: " + b.getType());
			
			if (b instanceof StartGameBean) {
//				System.out.println("PPL: StartGame Type: " + b.getType());
				// erwartet wird eine Antwort auf eine vom Server gesendete IQ
				// -> Fehlerbehandlung!
				StartGameBean bean = (StartGameBean) b;
				if (bean.getType() == XMPPBean.TYPE_SET) {
//					System.out.println("PPL: StartGameBean vom typ SET empfangen von: " + bean.getFrom());
				} else if (bean.getType() == XMPPBean.TYPE_RESULT) {
					System.out
							.println("PPL: StartGameBean vom typ RESULT empfangen von: " + bean.getFrom());
					player.startGame();
					player.acknowledgeBean(bean);
				} else if (bean.getType() == XMPPBean.TYPE_ERROR) {
					System.out.println("PPL: StartGameBean vom typ ERROR empfangen");
				}
...
//				System.out.println("PPL: QuitGame Type: " + b.getType());
				...
				if (bean.getType() == XMPPBean.TYPE_SET) {
					System.out.println("PPL: QuitBean vom typ SET empfangen von: " + bean.getFrom());
					...
					player.acknowledgeBean(bean);
					System.out.println("PPL: QuitBean vom typ RESULT empfangen");
				} else if (bean.getType() == XMPPBean.TYPE_ERROR) {
					player.acknowledgeBean(bean);
				}
...
//				System.out.println("PPL: GoThere Type: " + b.getType());
				...
				if (bean.getType() == XMPPBean.TYPE_SET) {
					System.out.println("PPL: GoThereBean vom typ SET empfangen");
					if(player.getTeamMate().goThere(bean)){
						player.sendAcknowledgement(bean);
					}else{
						player.sendError(bean, "");
					}
				} else if (bean.getType() == XMPPBean.TYPE_RESULT) {
					System.out.println("PPL: GoThereBean vom typ RESULT empfangen");
...
//				System.out.println("PPL: StartRound Type: " + b.getType());
				StartRoundBean bean = (StartRoundBean) b;
				if (bean.getType() == XMPPBean.TYPE_RESULT) {
//					System.out
//							.println("PPL: StartRoundBean vom typ RESULT empfangen von :" + bean.getFrom());
					player.acknowledgeBean(bean);
				} else if (bean.getType() == XMPPBean.TYPE_ERROR) {
					System.out
...
//				System.out.println("PPL: UncoverCard Type: " + b.getType());
				UncoverCardBean bean = (UncoverCardBean) b;
				if (bean.getType() == XMPPBean.TYPE_SET) {
					System.out.println("PPL: UncoverCardBean vom typ SET empfangen von: " + bean.getFrom());
					try {
						Date timeStamp = LocPairsDateFormat.getFormat().parse(bean
								.getTimeStamp());
						// execute player.uncoverCard() if successful send acknowledgment if not send errorIQ
					...
					System.out.println("PPL: UncoverCardBean vom typ RESULT empfangen");
					//player.acknowledgeBean(bean);
				} else if (bean.getType() == XMPPBean.TYPE_ERROR) {
					System.out.println("PPL: UncoverCardBean vom typ ERROR empfangen");
					//player.acknowledgeBean(bean);
				}
				
			} else if (b instanceof KeepAliveBean) {
//				System.out.println("PPL: KeepAlive Type: " + b.getType());
				KeepAliveBean bean = (KeepAliveBean) b;
				if (bean.getType() == XMPPBean.TYPE_SET) {
//					System.out.println("PPL: KeepAliveBean vom typ SET empfangen " + bean.getFrom());
					if(player.keepAlive())player.sendAcknowledgement(bean);
					player.setActualPosition(bean.getPosition());
				}else if(bean.getType() == XMPPBean.TYPE_GET){
					System.out.println("PPL: KeepAliveBean vom typ GET empfangen");
				}else if(bean.getType() == XMPPBean.TYPE_RESULT){
					System.out.println("PPL: KeepAliveBean vom typ RESULT empfangen");
				}else if(bean.getType() == XMPPBean.TYPE_ERROR){
					System.out.println("PPL: KeepAliveBean vom typ ERROR empfangen");
				}
				
			} else if (b instanceof PlayerUpdateBean) {
//				System.out.println("PPL: PlayerUpdate Type: " + b.getType());
...
//				System.out.println("PPL: JoinGame von: " + b.getFrom());
				...
				if (bean.getType() == XMPPBean.TYPE_RESULT) {
					System.out.println("PPL: JoinGameBean vom typ RESULT empfangen ERROR");
				}
				if (bean.getType() == XMPPBean.TYPE_ERROR) {
					System.out.println("PPL: JoinGameBean vom typ Error empfangen ERROR");
				}
				if (bean.getType() == XMPPBean.TYPE_SET) {
