...
public abstract class GeometryHelper {
  ...
  public static int getDrehsinn(Point[] points) {
     ...
     */
    static private boolean pointInsideExtent(
        double x,
        double y,
        double ax,
        double ay,
    ...
    static public boolean extentsOverlap(Extent a, Extent b) throws Exception {
        ...
        return (
                pointInsideExtent(a_xmin, a_ymin, b_xmin, b_ymin, b_xmax, b_ymax)
                || pointInsideExtent(a_xmax, a_ymin, b_xmin, b_ymin, b_xmax, b_ymax)
                || pointInsideExtent(a_xmax, a_ymax, b_xmin, b_ymin, b_xmax, b_ymax)
                || pointInsideExtent(a_xmin, a_ymax, b_xmin, b_ymin, b_xmax, b_ymax)
                || pointInsideExtent(b_xmin, b_ymin, a_xmin, a_ymin, a_xmax, a_ymax)
                || pointInsideExtent(b_xmax, b_ymin, a_xmin, a_ymin, a_xmax, a_ymax)
                || pointInsideExtent(
                b_xmax,
                b_ymax,
                a_xmin,
                ...
                a_xmax,
                a_ymax)
                || Extent2Polygon(a).intersects(Extent2Polygon(b)));
    }

    static public Polygon Extent2Polygon(Extent ext) throws Exception {
...
//        return GeometryHelper.isWithin(GeometryHelper.Extent2Polygon(queryExt), obj);
    }
    ...
    static public class IntersectionInfo {
        ...
        public void setSchnittTyp(int typ) {
            this.typ = typ;
            if ((typ == UNENDLICH_SCHNITTPUNKT)
                || (typ == KEIN_SCHNITTPUNKT)) {
                this.point = null;
            }
        }
	...
	public static boolean isWithin(Extent extent, Extent queryExt) {
		 double a_xmin = extent.getXmin();
	        ...
	        return pointInsideExtent(a_xmin, a_ymin, q_xmin, q_ymin, q_xmax, q_ymax)
			&& pointInsideExtent(a_xmin, a_ymax, q_xmin, q_ymin, q_xmax, q_ymax)
			&& pointInsideExtent(a_xmax, a_ymin, q_xmin, q_ymin, q_xmax, q_ymax)
			&& pointInsideExtent(a_xmax, a_ymax, q_xmin, q_ymin, q_xmax, q_ymax);
	}
  ...
  static public Ellipse2D.Double createCircle(Point center, double radius) {
  ...
  static public Extent returnExtentofAwtShape(Shape s) {
        return new Extent(
            s.getBounds2D().getMinX(),
            s.getBounds2D().getMinY(),
            s.getBounds2D().getMaxX(),
            s.getBounds2D().getMaxY());
    }
  ...
  public static boolean isWithin(GeoObject obj, GeoObject containerObj) throws Exception {
    // Konvertierung des GeoObjekts in eine Punktliste
    Point[] objPoints = obj.asPoints();

    // Pr�fung, ob containerObj vom Typ Ring (Ring, Polygon, Hole)
    if (containerObj instanceof Ring) {
      // Instanz Ring aus containerObj ableiten
      Ring ring = (Ring) containerObj;

      // Konvertieren des Rings in ein awtPolygon
      and.awt.geom.GeneralPath awtPoly = GeometryHelper.convertPolyToAwtPoly(ring);

      // Pr�fung und R�ckgabe, ob Punktliste innerhalb des awtPolygons.
      return GeometryHelper.isWithin(objPoints, awtPoly);

      // Wenn containerObj vom Typ MultiPolygon ...
    } else if (containerObj instanceof MultiPolygon) {
      // Instanz MultiPolygon aus containerObj ableiten
      MultiPolygon multi = (MultiPolygon) containerObj;

      //Schauen, ob die Fl�chen schon ges�ubert sind. Wenn nicht, dann die Chains nehmen...
      if (! multi.isClean()) {
      ...
      // Sonst die Polygone (Positivfl�chen) abfragen
      Polygon[] polys = multi.getPolygons();

      // Liste der Polygone durchlaufen
      for (int i = 0; i < polys.length; i++) {
        Polygon poly = polys[i];

        // Polygon in awtPolygon konvertieren
        and.awt.geom.GeneralPath awtPoly = GeometryHelper.convertPolyToAwtPoly(poly);

        // Wenn Punktliste innerhalb vom awtPolygon ...
        if (GeometryHelper.isWithin(objPoints, awtPoly)) {

          // Falls keine L�cher im Polygon vorhanden, dann isWithin = true;
          if (!multi.hasPolygonHoles(poly)) {
            return true;
          }

          // ... da L�cher vorhanden, zugeh�rige L�cher abfragen ...
          Hole[] holes = multi.getPolygonHoles(poly);

          // ... und Pr�fung aller L�cher
          for (int n = 0; n < holes.length; n++) {
            Hole h = holes[n];
            and.awt.geom.GeneralPath awtHole = GeometryHelper.convertPolyToAwtPoly(h);

            // Pr�fe, ob ein oder mehrere Punkte in ein Loch fallen
            for (int f = 0; f < objPoints.length; f++) {
              // F�llt ein Punkt in ein Loch wird isWithin = false !!!
      ...
      // Ende der Pr�fung aller Polygone im MultiPolygon
    }
  ...
  public static boolean intersects(Segment[] segments, Segment seg) {
    // Liste der Segmente durchlaufen
    for (int i = 0; i < segments.length; i++) {
  ...
  public static boolean intersects(GeoObject obj, Segment seg) {
    // Liefert GeoObject als Segmentliste
    Segment[] segments = obj.getSegments();

    if (segments != null) {
      // Schnittpr�fung f�r Segmentliste und Segment
      return intersects(segments, seg);
    }

    // Schnittpr�fung f�r Punktobjekte (Point und MultiPoint)
  ...
  //  /**
  //   * Liefert die Distanz zwischen Punkt und Segment. Wenn Distanz = 0, dann
  //   * liegt Punkt auf dem Segment
  //   * @param Point p
  //   * @param Segment s
  //   * @return boolean
  //   */
  //  public static double returnDistance(Point p, Segment s) {
  //    return returnDistance(s, p);
  //  }
  ...
  private static boolean touches(Segment segA, Segment segB, Point iP) {
    ...
    // iP darf nur Anfangs- oder Endpunkt eines der beiden Segmente sein.
  ...
  private static List dissolve(Segment[] segments, int recursionLevel) throws Exception {
    ...
    if (segments.length <= 3) {
      // Wenn sich 3 Segmente schneiden, muss ein St�tzpunkt verschoben werden.
      if (segments.length == 3) {
        ...
        try {
          ...
          // vielleicht Dreieck mit Fl�che 0...
  ...
  public static boolean extentOutside(Extent objExt, Extent queryExt) {
      ...
      return (quXmax < objXmin || quXmin > objXmax ||
              quYmax < objYmin || quYmin > objYmax);             
  }
  ...
  public static and.awt.geom.GeneralPath convertPolyToAwtPoly(Ring r) {
    // Segmente des Rings abfragen
    Segment[] segmente = r.getSegments();

    // GeneralPath erzeugen (s.a. Java 2D-Api, Package
    // java.awt.geom.GeneralPath)
    and.awt.geom.GeneralPath awtPoly = new and.awt.geom.GeneralPath();

    // Segmentliste durchlaufen und jedes Segment an den GeneralPath h�ngen
    // (append)
  ...
  public static and.awt.geom.Line2D.Double convertSegmentToAwtLine(Segment segment) {
    ...
    and.awt.geom.Line2D.Double line2d = new and.awt.geom.Line2D.Double(start.getX(), start.getY(), end.getX(), end
        .getY());
    ...
    for (int i = 0; i < segments.length; i++) {
      ...
      for (int k = 0; k < (i - 1); k++) {
        ...
        if ((p = aktSegment.returnIntersectionPoint(testSegment)) != null) {
          ...
          // F�ge 1. Teil des ersten Schnittsegments hinzu
          zweiterTeil.add(ergK[0]);

          // F�ge 2. Teil des zweiten Schnittsegments hinzu
          zweiterTeil.add(ergI[1]);

          // Segmente bis zum Ende �bernehmen
   ...
   * besitzen)
   */
  private static boolean collinear(double aX, double aY, double bX, double bY, double cX, double cY, double dX,
                                   double dY) {
  ...
  /**
   * Test, ob Punkt c auf dem Segment [a,b] liegt
   * @param aX X-Koordinate des Punktes a
   * @param aY Y-Koordinate des Punktes a
   * @param bX X-Koordinate des Punktes b
   * @param bY Y-Koordinate des Punktes b
   * @param cX X-Koordinate des Punktes c
   * @param cY Y-Koordinate des Punktes c
   * @return true, wenn Punkt c auf Segment [a,b] liegt
   */
  ...
  public static IntersectionInfo generateIntersectionInfo(Segment s1, Segment s2) {
     IntersectionInfo sI = new IntersectionInfo();
        ...
        /* Seien x1,y1 Koordinaten von P1, x2,y2 Koordinaten von P2, ...
        Sei S1 Gerade durch P1 und P2, S2 Gerade durch P3 und P4.
        Seien xs,ys Koordinaten des Schnittpunktes S. */
        ...
        if (nenner == 0.0) {
            // entweder:
            // Fall #1: eine von beiden ist Punkt (Segment mit Null Ausdehnung)
            // oder parallel
            if ((ax == bx) && (ay == by)) {
                // #1 Segment (a,b)
                ...
                if (collinear(ax, ay, bx, by, cx, cy, dx, dy)) {
                    if (between(ax, ay, bx, by, cx, cy)
                        ...
                        // Ueberlappung --> unendlich viele Schnittpunkte
                        sI.setSchnittTyp(
                            IntersectionInfo.UNENDLICH_SCHNITTPUNKT);
                    } else {
                        // Parallelitaet, kein Schnitt
   ...
   static private boolean Quick_collinear(
        double a_x,
        double a_y,
        double b_x,
        double b_y,
    ...
    static private boolean Quick_Xor(boolean a, boolean b) {
        return !(a && b) && (a || b);
    }

    static private boolean Quick_Left(
        double a_x,
        double a_y,
        double b_x,
        double b_y,
    ...
    static private double Quick_Area2(
        double a_x,
        double a_y,
        double b_x,
        double b_y,
        ...
        double c_y) {
        return (
            ((a_x * b_y) - (a_y * b_x) + (a_y * c_x))
                - (a_x * c_y)
                + (b_x * c_y))
            - (c_x * b_y);
    }

    static private boolean Quick_betweenInner(
        double a_x,
        double a_y,
        double b_x,
        double b_y,
        ...
        if (a_x != b_x) {
            return (
                ((a_x < c_x) && (c_x < b_x)) || ((a_x > c_x) && (c_x > b_x)));
        } else {
            return (
                ((a_y < c_y) && (c_y < b_y)) || ((a_y > c_y) && (c_y > b_y)));
        }
    }

    static private boolean Quick_intersectProp(
        double a_x,
        double a_y,
        double b_x,
        double b_y,
        ...
        double d_y) {
        if (Quick_collinear(a_x, a_y, b_x, b_y, c_x, c_y)
            || Quick_collinear(a_x, a_y, b_x, b_y, d_x, d_y)
            || Quick_collinear(c_x, c_y, d_x, d_y, a_x, a_y)
            || Quick_collinear(c_x, c_y, d_x, d_y, b_x, b_y)) {
            return false;
        }

        return Quick_Xor(
            Quick_Left(a_x, a_y, b_x, b_y, c_x, c_y),
            Quick_Left(a_x, a_y, b_x, b_y, d_x, d_y))
            && Quick_Xor(
                Quick_Left(c_x, c_y, d_x, d_y, a_x, a_y),
                Quick_Left(c_x, c_y, d_x, d_y, b_x, b_y));
    }

  static public boolean intersectsInner(GeoObject o, Segment s) {
        boolean is = false;
        ...
        if (segments != null) {
            ...
            //Line2D.Double line1 = GeometryHelper.convertSegmentToAwtLine(s);
            for (int i = 0; i < segments.length; i++) {
                //Line2D.Double line2 = GeometryHelper.convertSegmentToAwtLine(segments[i]);
                ...
                if (Quick_intersectProp(a_x,
                    ...
                    Quick_betweenInner(a_x, a_y, b_x, b_y, c_x, c_y)
                        || Quick_betweenInner(a_x, a_y, b_x, b_y, d_x, d_y)
                        || Quick_betweenInner(c_x, c_y, d_x, d_y, a_x, a_y)
                        || Quick_betweenInner(c_x, c_y, d_x, d_y, b_x, b_y)) {
                    return true;
                } else {
                    return false;
                }
